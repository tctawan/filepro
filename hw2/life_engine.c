...
{	
	...
	if(is_in_range(board, row , col) == 1){
...
{
	int count = 0;
	count += is_alive(board,row-1,col);
	count += is_alive(board,row+1,col);
	count += is_alive(board,row,col-1);	
	count += is_alive(board,row,col+1);
	count += is_alive(board,row-1,col-1);
	count += is_alive(board,row-1,col+1);
	count += is_alive(board,row+1,col-1);
	count += is_alive(board,row+1,col+1);
...
void make_next_board(life_board current, life_board next)
{
	for(int row = 0 ; row < current.num_rows ; row++){
		for(int col = 0 ; col < current.num_cols; col++){
			int nbrs = count_live_nbrs(current,row,col);
			if(is_alive(current,row,col)){
				if(nbrs == 2 || nbrs == 3){
